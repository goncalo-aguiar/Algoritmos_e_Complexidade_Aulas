...
// A: Esta ordem é a melhor neste caso porque é mais fácil de eliminar os últimos elementos da árvore primeiro e na travessia em pós-ordem
// são esses elementos que são eliminados em primeiro lugar. 
...
void* BSTreeSearch(const BSTree* header, const void* item) {
  ...
  while (current != NULL) {
    ...
    int x = header->compare(item,current->item);
    if(x==0){
      return current->item;
    }
    else if(x>0){
...
// A:Esta ordem é a melhor neste caso porque como a árvore está ordenada da esquerda para a direita e queremos que a pilha também
// esteja ordenada é mais facil percorrer a árvore através da travessia em ordem e ir adicionando os elementos à pilha começando 
// no valor minimo.
